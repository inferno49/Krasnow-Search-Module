...
if ($letter)
{
	...
	/*----------------------------------new-------------------------------*/
			...
			// keep only those nicknames that have the first letter = $letter:
		    $nick_neuron= NULL;
	...
	if ($temporary_search == 1)
{
    ...
    for($i2=0; $i2<$n_total_name; $i2++)
		...
		
		{
		  ...
		  $nickname_id[$n_neurons] = $type_1 ->getName_neuron_array($i2);
		  $type_1 -> retrive_id_by_name($nickname_id[$n_neurons]);
		  $id_neuron = $type_1 -> getID_namearray($n_neurons);
		  		$value_from_type++;
		break;		
	}
	...
	for($i3=0; $i3<$n_total_syn; $i3++)
		{
		  ...
		  $id_synonym =  $synonym_1 -> getID_array($n_neurons);
		  $synonymtyperel -> retrive_type_id_by_syn_id($id_synonym);
		  $id_neuron= $synonymtyperel -> getID_type_array($n_neurons);
	}
	}
	}
